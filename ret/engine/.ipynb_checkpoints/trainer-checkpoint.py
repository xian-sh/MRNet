...
from ret.utils.comm import get_world_size, synchronize
from ret.utils.metric_logger import MetricLogger
...
def do_train(
    ...
    vid_length = cfg.MODEL.RET.NUM_CLIPS
#     learn_vid_ret = torch.nn.Embedding(vid_length*vid_length, 1)
#     sigmoid = torch.nn.Sigmoid()
            
#     learn_ret = learn_vid_ret.weight.reshape(vid_length, vid_length).cuda()
#     learn_ret = sigmoid(learn_ret)
#     diag_mask = torch.diag(torch.ones(vid_length)).cuda()
#     video_ret = (1. - diag_mask) * learn_ret
#     overall_min = 0.15 * torch.mean(torch.abs(video_ret)) * cfg.SOLVER.BATCH_SIZE
#     logger.info(f"The min sparse value is: {overall_min}")
    overall_min = cfg.SOLVER.LOSS_RATIO * cfg.SOLVER.BATCH_SIZE * cfg.SOLVER.SPARSE_WEIGHT
    logger.info(f"The min sparse value is: {overall_min}")
...
#                 param.requires_grad_(True)
        ...
        for iteration, (batches, idx) in enumerate(data_loader):
            ...
            optimizer.zero_grad()
            contr_weight = cfg.MODEL.RET.LOSS.CONTRASTIVE_WEIGHT
            loss_vid, loss_sent, loss_iou, loss_sp = model(batches, cur_epoch=epoch)
            loss_vid, loss_sent = loss_vid * contr_weight, loss_sent * contr_weight
            loss_sp = loss_sp * cfg.SOLVER.SPARSE_WEIGHT
            meters.update(loss_vid=loss_vid.detach(), loss_sent=loss_sent.detach(), loss_iou=loss_iou.detach(), loss_sp=loss_sp.detach())
            
            loss = 0
            if epoch <= cfg.SOLVER.ONLY_IOU:
                loss += loss_iou
                loss += loss_sent + loss_vid + loss_sp
                if loss_sp >= overall_min:
                    loss += loss_sp
            else:
                loss += loss_iou
                loss += (loss_sent + loss_vid + loss_sp) * 0.01
                if loss_sp >= overall_min:
                    loss += loss_sp * 0.01
        ...
        if checkpoint_period != -1 and epoch % checkpoint_period == 0:
            checkpointer.save(f"{cfg.MODEL.RET.FEAT2D.NAME}_model_{epoch}e", **arguments)
