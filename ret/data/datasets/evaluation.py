...
import logging
import torch
from ret.data.datasets.utils import iou, score2d_to_moments_scores
from ret.utils.comm import is_main_process
...
def evaluate(cfg, dataset, predictions, nms_thresh, epoch, recall_metrics=(1, 5)):
    ...
    logger = logging.getLogger("ret.inference")
    ...
    for idx, result2d in tqdm(enumerate(predictions)):   # each video
        ...
        relults['scores'] = torch.stack(relults['scores'], dim=0).mean(dim=0)
        s.append(relults['scores'])
        res.append(relults)
    ...
    for h in x:
        ...
        res_new.update({
            f"{res[h]['vid']}": {
                ...
                'pred_scores': res[h]['scores'].tolist()[0],
                }
                })
