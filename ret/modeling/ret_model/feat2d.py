...
def build_feat2d(cfg):
    pooling_counts = cfg.MODEL.RET.FEAT2D.POOLING_COUNTS  # [15,8,8] anet, [15] charades
    num_clips = cfg.MODEL.RET.NUM_CLIPS  # 64 anet, 16 charades
    hidden_size = cfg.MODEL.RET.FEATPOOL.HIDDEN_SIZE  # 512
    if cfg.MODEL.RET.FEAT2D.NAME == "conv":
        return SparseConv(pooling_counts, num_clips, hidden_size)
    elif cfg.MODEL.RET.FEAT2D.NAME == "pool":
        return SparseMaxPool(pooling_counts, num_clips)
    elif cfg.MODEL.RET.FEAT2D.NAME == "concat":
        return SparseConcat(pooling_counts, num_clips)
    elif cfg.MODEL.RET.FEAT2D.NAME == "conv_b":
        return SparseConv_B(pooling_counts, num_clips, hidden_size)
    elif cfg.MODEL.RET.FEAT2D.NAME == "pool_b":
        return SparseMaxPool_B(pooling_counts, num_clips)
    elif cfg.MODEL.RET.FEAT2D.NAME == "conv_c":
        return SparseConv_C(pooling_counts, num_clips, hidden_size)
    elif cfg.MODEL.RET.FEAT2D.NAME == "pool_c":
        return SparseMaxPool_C(pooling_counts, num_clips)
    else:
        raise NotImplementedError("No such feature 2d method as %s" % cfg.MODEL.RET.FEAT2D.NAME)
