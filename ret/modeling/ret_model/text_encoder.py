...
class DistilBert(nn.Module):
    def __init__(self, input_dim, joint_space_size, dataset):
        super().__init__()

        # self.bert = ASTModel.from_pretrained("MIT/ast-finetuned-audioset-10-10-0.4593")
        self.fc_out1 = nn.Linear(input_dim, joint_space_size)
        self.fc_out2 = nn.Linear(input_dim, joint_space_size)
...
def build_text_encoder(cfg):
    joint_space_size = cfg.MODEL.RET.JOINT_SPACE_SIZE
    dataset_name = cfg.DATASETS.NAME
    input_dim = cfg.MODEL.RET.INPUT_TEXT_DIM
    return DistilBert(input_dim, joint_space_size, dataset_name)
