...
def attention(q, k, v, attn_mask, dropout, att_softmax):
    B, Nt, E = q.shape
    q = q / math.sqrt(E)
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        attn_output, attn_output_weights = attention(q, k, v, attn_mask, self.dropout, self.att_softmax)
...
class RetLayer(nn.Module):
    ...
    def forward(self, src, txt_len):
        ...
        # Decay matrix
        D = self._get_D(vid_len, self.d_double).cuda()
        ...
        if self.att_mode == 'v':
            ...
            ret_mask[txt_len:, txt_len:] = D

            sparse_mask = torch.ones(seq_len, seq_len).cuda()
            sparse_mask[txt_len:, txt_len:] = learn_mask
        ...
        if self.att_mode == 'v':
            ...
            ret_V, _ = self.self_attn(Q, K, V, mask=mask)
            ...
            out = torch.cat([txt_V, vid_V], dim=1)

        else:
            raise 'error! the attention mode only can be \'v\' or \'all\' '

        return out, loss_sparsity
